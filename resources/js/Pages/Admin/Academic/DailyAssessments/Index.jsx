...
export default function Index({ auth, assessments, journals, classes, subjects, filters }) {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('admin.academic.daily-assessments.store'), {
            onSuccess: () => {
                    ...
                    <div className="bg-white/80 backdrop-blur-xl p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
                        ...
                        <div className="overflow-x-auto rounded-2xl border border-slate-100">
                            <table className="hidden md:table w-full text-sm text-left text-slate-600">
                                ...
                                <tbody>
                                    {assessments.data && assessments.data.map((j) => (
                                        <tr key={j.id} className="hidden md:table-row border-b border-slate-50 hover:bg-slate-50/80 transition-colors group">
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <Link
                                                    href={route('admin.academic.daily-assessments.show', j.id)}
                ...
                <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6 max-h-screen overflow-y-auto">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700">Pilih Jurnal Mengajar</label>
                                <select className="mt-1 block w-full border-gray-300 rounded-md" value={data.journal_id} onChange={e => setData('journal_id', e.target.value)} required>
                                    <option value="">-- Pilih Jurnal --</option>
                                    {journals.map(j => (
                                        <option key={j.id} value={j.id}>{j.date} | Pertemuan {j.session_number} - {j.topic} ({j.school_class?.name})</option>
                                    ))}
                                </select>
                            </div>
