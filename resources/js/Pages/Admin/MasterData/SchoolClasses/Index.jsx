...
import { Search, Plus, Edit, Trash2, User, Users, BookOpen, UserCheck, X, Printer, Save } from 'lucide-react';
import Pagination from '@/Components/Pagination';

export default function Index({ auth, schoolClasses, filters }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            router.delete(route('admin.master.schoolClasses.destroy', id), { preserveScroll: true });
        }
    };
    ...
    const openEditModal = (schoolClass) => {
        setIsEditing(true);
        setCurrentId(schoolClass.id);
        setData({
            nama_lengkap: schoolClass.nama_lengkap,
    ...
    const submitForm = (e) => {
        e.preventDefault();
        if (isEditing) {
            put(route('admin.master.schoolClasses.update', currentId), {
                preserveScroll: true,
                onSuccess: () => {
            ...
            post(route('admin.master.schoolClasses.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    setIsModalOpen(false);
                    reset();
    ...
    return (
        ...
        >
            <Head title="Master SchoolClasses" />
                        ...
                        <form onSubmit={handleSearch} className="w-full md:w-96 relative">
                            <div className="relative flex items-center">
                                ...
                                <input 
                                    ...
                                    placeholder="Cari nama atau Nama Kelas..." 
                        ...
                        <div className="p-4 sm:p-6 md:p-8">
                            {schoolClasses.data.length === 0 ? (
                                <div className="text-center py-16 bg-white/50 rounded-3xl border border-dashed border-slate-200">
                                    ...
                                    <p className="text-slate-500 text-lg font-medium">Tidak ada data kelas ditemukan.</p>
                                        ...
                                        <div className="inline-block min-w-full align-middle">
                                            <table className="min-w-full divide-y divide-slate-100">
                                                <thead className="bg-slate-50/50 backdrop-blur-sm border-b border-slate-100">
                                                    <tr>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Kelas</th>
                                                ...
                                                {schoolClasses.data.map((item) => (
                                                    <motion.tr variants={itemVariants} key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            ...
                                                            <div className="text-xs text-slate-500">{item.grade_level}</div>
                                                        </td>
                                        ...
                                        <motion.div variants={itemVariants} key={item.id} className="bg-white p-4 rounded-[1.5rem] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-slate-50 flex flex-col relative overflow-hidden">
                                            ...
                                            <div className="flex items-center gap-3.5 ml-1">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-xs font-medium text-slate-500 mt-0.5 truncate">Tingkat: <span className="text-slate-700 font-semibold">{item.grade_level}</span></p>
                                                </div>
                                            </div>
                                ...
                                {/* Pagination */}
                                <Pagination links={schoolClasses.links} />
                            </>
                        )}
                        </div>
                    </motion.div>
