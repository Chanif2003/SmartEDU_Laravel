...
export default function TeachersTab({ auth, tabData: teachers, filters }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            router.delete(route('admin.master.teachers.destroy', id), { preserveScroll: true });
        }
    };
    ...
    const submitForm = (e) => {
        e.preventDefault();
        if (isEditing) {
            put(route('admin.master.teachers.update', currentId), {
                preserveScroll: true,
                onSuccess: () => {
            ...
            post(route('admin.master.teachers.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    setIsModalOpen(false);
                    reset();
                                        ...
                                        <div className="inline-block min-w-full align-middle">
                                            <table className="min-w-full divide-y divide-slate-100">
                                                <thead className="bg-slate-50/50 backdrop-blur-sm border-b border-slate-100">
                                                    <tr>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Guru</th>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">NIP / NUPTK</th>
                                                ...
                                                {teachers.data.map((item) => (
                                                    <motion.tr variants={itemVariants} key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            ...
                                                            <div className="text-xs text-slate-500">{item.nuptk}</div>
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                            <div className="flex justify-end gap-2">
                                                                <Link href={route('admin.master.teachers.show', item.id)} className="p-2.5 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors" title="Detail">
                                    ...
                                    {teachers.data.map((item) => (
                                        <motion.div variants={itemVariants} key={item.id} className="bg-white p-4 rounded-[1.5rem] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-slate-50 flex flex-col relative overflow-hidden">
                                            ...
                                            <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-50 ml-1">
                                                ...
                                                <Link href={route('admin.master.teachers.show', item.id)} className="p-2 text-slate-400 bg-slate-50 hover:bg-blue-50 hover:text-blue-600 rounded-xl active:scale-95 transition-all" title="Detail">
                        ...
                        >
                            ...
                            <form onSubmit={submitForm} className="p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">NUPTK</label>
                                        <input 
                                            type="text" 
                                            value={data.nuptk} 
                                            ...
                                            className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-800 focus:ring-2 focus:ring-brand-500 outline-none"
                                        />
                                        {errors.nuptk && <div className="text-red-500 text-xs mt-1">{errors.nuptk}</div>}
                                    </div>
                                </div>
                                <div>
