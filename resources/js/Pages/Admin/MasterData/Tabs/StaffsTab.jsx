...
export default function StaffsTab({ auth, tabData: staffs, filters }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            router.delete(route('admin.master.staffs.destroy', id), { preserveScroll: true });
        }
    };
    ...
    const submitForm = (e) => {
        e.preventDefault();
        if (isEditing) {
            put(route('admin.master.staffs.update', currentId), {
                preserveScroll: true,
                onSuccess: () => {
            ...
            post(route('admin.master.staffs.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    setIsModalOpen(false);
                    reset();
                        ...
                        <div className="p-4 sm:p-6 md:p-8">
                            {staffs.data.length === 0 ? (
                                <div className="text-center py-16 bg-white/50 rounded-3xl border border-dashed border-slate-200">
                                    ...
                                    <p className="text-slate-500 text-lg font-medium">Tidak ada data guru ditemukan.</p>
                                        ...
                                        <div className="inline-block min-w-full align-middle">
                                            <table className="min-w-full divide-y divide-slate-100">
                                                <thead className="bg-slate-50/50 backdrop-blur-sm border-b border-slate-100">
                                                    <tr>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Staf</th>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">NIP / Jabatan</th>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Jenis Kelamin</th>
                                            ...
                                            <tbody className="divide-y divide-slate-100">
                                                {staffs.data.map((item) => (
                                                    <motion.tr variants={itemVariants} key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <div className="text-sm font-bold text-slate-800">{item.nip}</div>
                                                            <div className="text-xs text-slate-500">{item.jabatan}</div>
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <span className={`px-4 py-1.5 inline-flex text-xs font-black uppercase tracking-wider rounded-full ${item.jenis_kelamin === 'Laki-laki' ? 'bg-blue-100 text-blue-700' : 'bg-pink-100 text-pink-700'}`}>
                                                                {item.jenis_kelamin}
                                                            </span>
                                                        </td>
                                ...
                                <div className="grid grid-cols-1 gap-3 md:hidden mb-6">
                                    {staffs.data.map((item) => (
                                        <motion.div variants={itemVariants} key={item.id} className="bg-white p-4 rounded-[1.5rem] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-slate-50 flex flex-col relative overflow-hidden">
                                            ...
                                            <div className={`absolute left-0 top-0 bottom-0 w-1 ${item.jenis_kelamin === 'Laki-laki' ? 'bg-blue-400' : 'bg-pink-400'}`}></div>
                                            ...
                                            <div className="flex items-center gap-3.5 ml-1">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-xs font-medium text-slate-500 mt-0.5 truncate">NIP: <span className="text-slate-700 font-semibold">{item.nip}</span></p>
                                                </div>
                                            </div>
                                            
                                            <div className="flex items-center gap-2 mt-4 pt-3 border-t border-slate-50 ml-1">
                                                <span className={`mr-auto px-2.5 py-1 text-[10px] font-black uppercase tracking-widest rounded-lg ${item.jenis_kelamin === 'Laki-laki' ? 'bg-blue-50 text-blue-600' : 'bg-pink-50 text-pink-600'}`}>
                                                    {item.jenis_kelamin}
                                                </span>
                        ...
                        >
                            <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-slate-50/50">
                                <h3 className="font-black text-xl text-slate-800">
                                    {isEditing ? 'Edit Staf' : 'Tambah Staf'}
                                </h3>
                            ...
                            <div className="p-6">
                                <div id="qr-print-area" className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50">
                                    ...
                                    <div className="mt-6 flex flex-col items-center gap-1 text-slate-500">
                                        <p className="text-sm font-semibold tracking-widest uppercase">NIP</p>
                                        <p className="text-xl font-bold text-slate-700">{selectedQr.nip}</p>
                                    </div>
                                </div>
                            </div>
