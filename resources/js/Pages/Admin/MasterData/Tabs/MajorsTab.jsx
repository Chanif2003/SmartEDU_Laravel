...
export default function MajorsTab({ auth, tabData: majors, filters }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            router.delete(route('admin.master.majors.destroy', id), { preserveScroll: true });
        }
    };
    ...
    const submitForm = (e) => {
        e.preventDefault();
        if (isEditing) {
            put(route('admin.master.majors.update', currentId), {
                preserveScroll: true,
                onSuccess: () => {
            ...
            post(route('admin.master.majors.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    setIsModalOpen(false);
                    reset();
                        ...
                        <form onSubmit={handleSearch} className="w-full md:w-96 relative">
                            <div className="relative flex items-center">
                                ...
                                <input 
                                    ...
                                    placeholder="Cari nama atau Nama Jurusan..." 
                        ...
                        <div className="p-4 sm:p-6 md:p-8">
                            {majors.data.length === 0 ? (
                                <div className="text-center py-16 bg-white/50 rounded-3xl border border-dashed border-slate-200">
                                    ...
                                    <p className="text-slate-500 text-lg font-medium">Tidak ada data jurusan ditemukan.</p>
                                        ...
                                        <div className="inline-block min-w-full align-middle">
                                            <table className="min-w-full divide-y divide-slate-100">
                                                <thead className="bg-slate-50/50 backdrop-blur-sm border-b border-slate-100">
                                                    <tr>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Jurusan</th>
                                        ...
                                        <motion.div variants={itemVariants} key={item.id} className="bg-white p-4 rounded-[1.5rem] shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-slate-50 flex flex-col relative overflow-hidden">
                                            ...
                                            <div className="flex items-center gap-3.5 ml-1">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-xs font-medium text-slate-500 mt-0.5 truncate">Deskripsi: <span className="text-slate-700 font-semibold">{item.description}</span></p>
                                                </div>
                                            </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <form onSubmit={submitForm} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">Nama Jurusan</label>
                                    <input 
                                        type="text" 
                                        value={data.name} 
                                ...
                                <div>
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-800 focus:ring-2 focus:ring-brand-500 outline-none min-h-[100px]"
                                    />
