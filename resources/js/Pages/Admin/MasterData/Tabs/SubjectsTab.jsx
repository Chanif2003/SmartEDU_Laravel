...
import { Search, Plus, Edit, Trash2, User, Users, Library, UserCheck, X, Printer, Save } from 'lucide-react';
import Pagination from '@/Components/Pagination';

export default function SubjectsTab({ auth, tabData: subjects, filters }) {
    ...
    const handleDelete = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus data ini?')) {
            router.delete(route('admin.master.subjects.destroy', id), { preserveScroll: true });
        }
    };
    ...
    const openEditModal = (subject) => {
        ...
        setData({
            nama_lengkap: subject.nama_lengkap,
            name: subject.name,
    ...
    const submitForm = (e) => {
        e.preventDefault();
        if (isEditing) {
            put(route('admin.master.subjects.update', currentId), {
                preserveScroll: true,
                onSuccess: () => {
            ...
            post(route('admin.master.subjects.store'), {
                preserveScroll: true,
                onSuccess: () => {
                    setIsModalOpen(false);
                    reset();
                        ...
                        <form onSubmit={handleSearch} className="w-full md:w-96 relative">
                            <div className="relative flex items-center">
                                ...
                                <input 
                                    ...
                                    placeholder="Cari nama atau Nama Mapel..." 
                        ...
                        <div className="p-4 sm:p-6 md:p-8">
                            {subjects.data.length === 0 ? (
                                <div className="text-center py-16 bg-white/50 rounded-3xl border border-dashed border-slate-200">
                                    ...
                                    <p className="text-slate-500 text-lg font-medium">Tidak ada data mata pelajaran ditemukan.</p>
                                        ...
                                        <div className="inline-block min-w-full align-middle">
                                            <table className="min-w-full divide-y divide-slate-100">
                                                <thead className="bg-slate-50/50 backdrop-blur-sm border-b border-slate-100">
                                                    <tr>
                                                        <th className="px-6 py-5 text-left text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Mata Pelajaran</th>
                        ...
                        >
                            ...
                            <form onSubmit={submitForm} className="p-6 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-bold text-slate-700 mb-1">Nama Mapel</label>
                                        <input 
                                            type="text" 
                                            value={data.name} 
